package com.blackcandle.baselib.event;
...
public class LiveDataBus {
    private volatile static LiveDataBus liveDataBus = new LiveDataBus();
    //存储所有LiveData的容器
    ...
    /**
     * 存和取一体的方法
     * 存在Key，则直接返回BusMutableLiveData对象
     * 不存在Key，则新建一个和Key对应的BusMutableLiveData
     * @param key
     * @param type
     * @param <T>
     * @return
    ...
    public class BusMutableLiveData<T> extends MutableLiveData<T> {
        ...
        // ⚠️ isViscosity=true能够拦截粘性事件，只有在onCreate函数中调用observe，才有效。
